class DashboardsController < UserApplicationController
  ...
  def show
    ...
    products_count = sql.execute("select p.name, count(oi.id) from products p, order_items oi, orders o where p.id = oi.product_id and oi.order_id = o.id and o.closed = 't' and o.generated_at >= '#{Time.zone.today.beginning_of_month}' group by p.id")
    ...
    last_month_end = Time.zone.today.beginning_of_month - 1.day 
