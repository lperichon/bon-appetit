class DashboardsController < UserApplicationController
  ...
  def show
    ...
    @products_pie_labels = sql.execute("select p.name, count(oi.id) from products p, order_items oi, orders o where p.id = oi.product_id and oi.order_id = o.id and o.generated_at > '#{Date.today.beginning_of_month}' group by p.id").collect {|d| d[0]}
    @products_pie_data = sql.execute("select p.name, count(oi.id) from products p, order_items oi, orders o where p.id = oi.product_id and oi.order_id = o.id and o.generated_at > '#{Date.today.beginning_of_month}' group by p.id").collect {|d| d[1]}
